...
(mejs.version = "2.16.4"),
  ...
  (mejs.HtmlMediaElementShim = {
    ...
    getTypeFromFile: function (a) {
      ...
      return (
        (/(mp4|m4v|ogg|ogv|m3u8|webm|webmv|flv|wmv|mpeg|mov)/gi.test(b)
          ? "video"
          : "audio") +
        "/" +
        this.getTypeFromExtension(b)
      );
    },
      ...
      (mejs.MediaElementPlayer.prototype = {
        ...
        setPlayerSize: function (b, c) {
          ...
          j.height(d.container.height() - d.controls.height()),
            k.css(
              "margin-top",
              "-" + (k.height() / 2 - d.controls.height() / 2).toString() + "px"
            );
        },
        setControlsSize: function () {
          var b = this,
            ...
            g =
              (b.controls.find(".mejs-time-current"),
              b.controls.find(".mejs-time-loaded"),
              e.siblings()),
            h = g.last(),
            i = null;
          if (b.container.is(":visible") && e.length && e.is(":visible")) {
            ...
            do
              e.width(d),
                ...
                "absolute" != h.css("position") && ((i = h.position()), d--);
            ...
            n = function (a) {
              ...
              (b = a.originalEvent.changedTouches
                ...
                e.duration &&
                  (b < c.left
                    ? (b = c.left)
                    : b > d + c.left && (b = d + c.left),
                  (i = b - c.left),
                  (f = i / d),
                  (h = 0.02 >= f ? 0 : f * e.duration),
                  o && h !== e.currentTime && e.setCurrentTime(h),
                  mejs.MediaFeatures.hasTouch ||
                    (k.css("left", i),
          ...
          c.children().last().find(".mejs-currenttime").length > 0
            ? a(
                f.options.timeAndDurationSeparator +
                  ...
                  (f.options.duration > 0
                    ? mejs.Utility.secondsToTimeCode(
                        f.options.duration,
                        f.options.alwaysShowHours || f.media.duration > 3600,
                        f.options.showTimecodeFrameCount,
        ...
        updateCurrent: function () {
          ...
          a.currenttime &&
            a.currenttime.html(
              mejs.Utility.secondsToTimeCode(
                a.media.currentTime,
                a.options.alwaysShowHours || a.media.duration > 3600,
          ...
          if ("native" == f.media.pluginType) {
            ...
            for (var j = 0, k = f.options.speeds.length; k > j; j++)
              i +=
                ...
                f.options.speeds[j] +
                '" ' +
                (f.options.speeds[j] == f.options.defaultSpeed
                  ? " checked"
                  : "") +
                ' /><label for="' +
                f.options.speeds[j] +
            ...
            (i += "</ul></div></div>"),
              ...
              (playbackspeed = f.options.defaultSpeed),
              h.on("click", 'input[type="radio"]', function () {
                var b = a(this).attr("value");
                (playbackspeed = b),
                  (e.playbackRate = parseFloat(b)),
                  g.find("button").html(b + f.options.speedChar),
                  g
        ...
        buildtracks: function (b, c, d, e) {
          if (0 !== b.tracks.length) {
            ...
            for (f = 0; f < b.tracks.length; f++)
              "subtitles" == b.tracks[f].kind && h++;
            for (
              g.options.toggleCaptionsButtonWhenOnlyOne && 1 == h
